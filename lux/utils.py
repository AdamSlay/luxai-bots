...
def closest_type_tile(tile_type: str, unit, game_state, player, obs) -> np.ndarray:
    type_tiles = obs["board"][tile_type]
    # units = game_state.units[player]
    # for uid, u in units.items():
    #     type_tiles[u.pos[0]][u.pos[1]] = 0
    if tile_type != "rubble":
        tile_locations = np.argwhere(type_tiles == 1)
    else:
        tile_locations = np.argwhere((game_state.board.rubble <= 50) & (game_state.board.rubble > 0))
